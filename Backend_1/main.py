    ...
    if ENABLE_PINECONE_LOGGING and PINECONE_API_KEY:
        try:
            ...
            if openai_client and pc_client:
                ...
                # NATIVE THREAD UNBLOCKING OVERRIDE: Leverage Pinecone's concurrent connection engine pooling
                index_target = pc_client.Index(PINECONE_INDEX_NAME)
                
                # Using async_req=True handles threading natively at the core layer
                ...
                # Securely finalize the transaction block to make sure the data transfers before the thread terminates
                async_result.get()
